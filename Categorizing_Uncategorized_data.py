...
for category in category_list:
    ...
    data = []
    target =[]
    list2 = os.listdir('./Uncategorized_data/'+category)
    for j in list2:
        fp = open('./Uncategorized_data/'+category+'/'+j,'r')
        data.append(fp.read())
        fp.close()

    predicted = text_clf_svm.predict(data)
    Categorized_data.append(zip(data,predicted))
fp = open('./Categorized_Uncategorized_data.txt','w')
fp.write(str(Categorized_data))
fp.close()